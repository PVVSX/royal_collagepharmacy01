...
import { requestsData } from "@/roles/shared/data";
...
const sb: Record<string, { variant: "warning" | "success" | "danger"; label: string }> = {
  pending: { variant: "warning", label: "รอดำเนินการ" }, approved: { variant: "success", label: "อนุมัติแล้ว" }, rejected: { variant: "danger", label: "ถูกปฏิเสธ" },
};
const bc: Record<string, string> = { pending: "border-l-warning", approved: "border-l-success", rejected: "border-l-danger" };
    ...
    <>
      <PageShell>
        ...
        <div className="flex flex-wrap gap-2 mb-4">{tabs.map((tab) => (
          <Badge key={tab.key} variant={activeTab === tab.key ? "brand" : "outline"} className="cursor-pointer text-xs px-3 py-1.5" onClick={() => setActiveTab(tab.key)}>{tab.key} ({tab.count})</Badge>
        ))}</div>
        ...
        <div className="space-y-3">{filtered.map((req) => { const s = sb[req.status]; return (
          <Card key={req.id} className={`card-shadow border-l-4 ${bc[req.status]} cursor-pointer hover:bg-muted/50 transition-colors`} onClick={() => { setSelectedRequest(req); setShowDetailModal(true); }}><CardContent className="p-4">
            <div className="flex items-start justify-between mb-2"><div><div className="flex items-center gap-2 mb-0.5"><span className="text-xs text-muted-foreground font-mono">{req.id}</span><Badge variant={s.variant} className="text-xs opacity-90 px-1.5 py-0">{s.label}</Badge></div><h3 className="text-sm font-semibold">{req.title}</h3><p className="text-xs text-muted-foreground mt-0.5">{req.type} · {req.date}</p></div></div>
            ...
            {selectedRequest && (
              <div className="space-y-4 text-sm mt-2">
                <div className="flex justify-between items-start border-b pb-3">
                  ...
                  <Badge variant={sb[selectedRequest.status].variant}>{sb[selectedRequest.status].label}</Badge>
                </div>
                  ...
                  <div className="space-y-3 pl-2 border-l-2 border-border ml-2">
                    {selectedRequest.progress.map((step: string, i: number) => (
                      <div key={i} className="relative pl-4">
                        ...
                        <p className={`text-xs ${i < selectedRequest.progress.length - (selectedRequest.status === 'pending' ? 1 : 0) ? 'font-medium' : 'text-muted-foreground'}`}>{step.replace(' ◷', '')} {step.includes('◷') && <span className="text-2xs text-warning font-normal ml-1">(กำลังดำเนินการ)</span>}</p>
                      </div>
                    ))}
                  </div>
                </div>
