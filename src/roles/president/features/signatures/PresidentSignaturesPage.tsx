  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6">
      ...
      <Card><CardContent className="space-y-4 px-4 md:px-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-semibold text-foreground">รายการทั้งหมด</h2><p className="mt-1 text-xs text-muted-foreground">{isReady ? `${pending.length} รายการ` : "กำลังโหลด"}</p></div><div className="relative w-full sm:w-80"><span className="material-symbols-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-lg text-muted-foreground">search</span><Input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ค้นหาเลขที่คำร้องหรือชื่อผู้ยื่น" aria-label="ค้นหาคำร้องรอลงนาม" className="h-11 rounded-xl pl-9 text-sm" /></div></div>
