  ...
  return (
    <PageShell size="full" className="space-y-6">
      ...
      <Card><CardContent className="space-y-4 px-4 md:px-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-semibold">เหตุการณ์ล่าสุด</h2><p className="mt-1 text-xs text-muted-foreground">ผู้ใช้ทั่วไปแก้ไขหรือลบประวัตินี้ไม่ได้</p></div><Input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ค้นหา Action, Resource, ผู้ใช้ หรือหลักฐาน" aria-label="ค้นหา Business Audit" className="h-11 rounded-xl text-sm sm:w-80" /></div>
